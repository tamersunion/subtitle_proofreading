...
def _load_prompt(category: str) -> str:
    ...
    # 3. 对应类别的 Guidelines
    rule_files = _CATEGORY_RULES[category]
    rule_content = "\n\n".join(
        (prompt_dir / rule_file).read_text(encoding="utf-8")
        for rule_file in rule_files
    )
    parts.append(f"<Guidelines>\n{rule_content}\n</Guidelines>")

    # 4. Glossary（综合检查作为参考，术语检查作为强制规范）
    glossary_content = (data_dir / "glossary.xml").read_text(encoding="utf-8")
    if category == "glossary":
        parts.append(glossary_content)
    else:
        parts.append(f"以下译名表仅供参考，用于理解既定译名和专有名词，不要进行术语一致性检查。\n{glossary_content}")
...
async def _check_chunk_pair(chunk: list[str], strParser: StrOutputParser) -> str:
    """对单个 chunk 执行综合检查与术语检查，各跑一次 AI 任务"""
    general_result = await _check_chunk_with_label(chunk, "general", strParser)
    glossary_result = await _check_chunk_with_label(chunk, "glossary", strParser)

    merged = "\n".join(r for r in (general_result, glossary_result) if r)
    return merged
...
async def check_ass(file: Path, chunk_size: int = 30, splitter: str = r"\N{\fnG-OTF Jo Shin Maru Go ProN M\fs45}", style: str = "Default") -> list[str]:
    ...
    for i in range(0, len(lines), chunk_size):
        ...
        response = await _check_chunk_pair(chunk, strParser)
